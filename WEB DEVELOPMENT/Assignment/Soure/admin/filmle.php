			...
			<div class="content col-sm-10">
				<table class="table"> 
				    <div style="padding: 20px 10px">
				    	<a href="./insertfilmle.php"><button class="btn btn-success"><span style="padding-right: 10px" class="glyphicon glyphicon-film"></span>Thêm phim lẻ mới</button></a>
				    </div>				
					<tr>
						<th></th>
						<th>Tên phim</th>
					...
					<?php 
					    ...
					    $convert_ngonngu = array("","Tiếng việt", "Lồng tiếng", "Thuyết minh","ViệtSub");
							...
							$sql = "SELECT * FROM phimle ORDER BY 1 DESC" . " LIMIT $start_from, $num_rec_per_page"; 
							$result = $conn->query($sql);
					...
					?>
							<tr>
							    ...
							    <td style="vertical-align: middle;"><a title="Xóa phim" href="deletefilmle.php?del=<?php echo $phim_id; ?>" onclick="return confirm('Bạn chắc chắn muốn xóa phim  <?php echo $tenphim; ?>?');"><button type="button" class="btn btn-danger">
						          <span class="glyphicon glyphicon-remove-sign"></span>
						        </button></a></td>
								<td style="vertical-align: middle;"><a title="Chỉnh sửa phim" href="editfilmle.php?edit=<?php echo $phim_id; ?>"><button type="button" class="btn btn-success">
					...
					<?php
						include './connectdatabase.php';
						$sql = "SELECT * FROM phimle ORDER BY 1 DESC";
						$result = $conn->query($sql);
                        ...
                        echo "<li><a href='./filmle.php?".$_SERVER['QUERY_STRING']."&page=1'>".'<span class="glyphicon glyphicon-fast-backward"></span>'."</a></li> "; // Goto 1st page  

                        for ($i=1; $i<=$total_pages; $i++) { 
                            echo "<li><a href='./filmle.php?".$_SERVER['QUERY_STRING']."&page=".$i."'>".$i."</a></li> "; 
                        }; 
                        echo "<li><a href='./filmle.php?".$_SERVER['QUERY_STRING']."&page=$total_pages'>".'<span class="glyphicon glyphicon-fast-forward">'."</a></li> "; // Goto last page
                        echo "</ul>";
					?>
			</div>
		</div>
